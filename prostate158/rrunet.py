...
class RRUNet3D(nn.Module):
    def __init__(
        ...
        self.input_conv = nn.Conv3d(
            in_channels, num_init_kernels, 3, stride=2, padding=1
        )
        self.output_conv = nn.Conv3d(
            num_init_kernels, out_channels, 1, stride=1, padding=0, bias=False
        )
        ...
        for _i in range(self.levels_down):
            ...
            in_c = (
                num_init_kernels * 2**_i
                if _i == 0
                else num_init_kernels * 2 ** (_i - 1)
            )
        ...
        if self.attention is True:
            ...
            for _i in range(self.levels_up):
                ...
                self.attention_blocks.append(
                    AttentionBlock(F_g=F_g, F_l=F_l, F_int=F_int)
                )
