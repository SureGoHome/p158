...
def pred_logger(engine):
    ...
    if not os.path.exists(root):
        os.makedirs(root)
        torch.save(engine.state.output[0]["label"], os.path.join(root, f"label.pt"))
        torch.save(engine.state.output[0]["image"], os.path.join(root, f"image.pt"))
...
def get_evaluator(
    ...
    # allow overriding AMP via config.amp to avoid known cuDNN/BN issues on some GPUs
    use_amp = getattr(config, "amp", USE_AMP)
...
class SegmentationTrainer(monai.engines.SupervisedTrainer):
    ...
    def __init__(
        ...
        # allow overriding AMP via config.amp to avoid known cuDNN/BN issues on some GPUs
        use_amp = getattr(self.config, "amp", USE_AMP)
        super().__init__(
            device=config.device,
